...
void main() {
  ...
  const tFailure = ApiFailure(message: 'message', statusCode: 400);
  ...
  group('createUser', () {
    blocTest<AuthenticationCubit, AuthenticationState>(
      ...
      act: (cubit) => cubit.createUser(
        ...
        fullname: tCreateUserParams.fullname,
    ...
    blocTest<AuthenticationCubit, AuthenticationState>(
        ...
        build: () {
          when(() => createUser(any())).thenAnswer((_) async => const Left(tFailure));
          return cubit;
        },
        act: (cubit) => cubit.createUser(
            createdAt: tCreateUserParams.createdAt,
        ...
        expect: () => [
          const CreatingUser(),
          AuthenticationError(tFailure.errorMessage)
        ],
        verify: (_) {
          verify(() => createUser(tCreateUserParams)).called(1);
          verifyNoMoreInteractions(createUser);
  ...
  group('getUsers', () {
    ...
    blocTest<AuthenticationCubit, AuthenticationState>(
        ...
        build: () {
          when(() => getUsers()).thenAnswer((_) async => const Left(tFailure));
          return cubit;
        },
        act: (cubit) => cubit.getUsers(),
        expect: () => [
